<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <link href="https://cdn.jsdelivr.net/npm/tailwindcss@2.2.19/dist/tailwind.min.css" rel="stylesheet">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link href="https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;500;600;700&display=swap" rel="stylesheet">
    <title>Cart by Megat Irfan</title>
</head>
<style>
  body {
    font-family: 'Space Grotesk', sans-serif;
  }
</style>
<body>
  <form action="payment.php" method="post">
  <section class="h-screen bg-gray-100 py-12 sm:py-16 lg:py-20">
    <div class="mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-center">
        <h1 class="text-2xl font-semibold text-gray-900">Your Cart</h1>
      </div>
     
      <div class="mx-auto mt-8 max-w-2xl md:mt-12">
        <div class="bg-white shadow">
          <div class="px-4 py-6 sm:px-8 sm:py-10">
            <div class="flow-root">
              <ul class="-my-8">
                <li class="flex flex-col space-y-3 py-6 text-left sm:flex-row sm:space-x-5 sm:space-y-0">
                  <div class="shrink-0">
                    <img class="h-24 w-24 max-w-full rounded-lg object-cover" src="https://static.nike.com/a/images/t_PDP_1280_v1/f_auto,q_auto:eco/b7d9211c-26e7-431a-ac24-b0540fb3c00f/air-force-1-07-shoes-WrLlWX.png" alt="" />
                  </div>
  
                  <div class="relative flex flex-1 flex-col justify-between">
                    <div class="sm:col-gap-5 sm:grid sm:grid-cols-2">
                      <div class="pr-8 sm:pr-5">
                        <p class="text-base font-semibold text-gray-900">Nike Air Force 1</p>
                        <p class="mx-0 mt-1 mb-0 text-sm text-gray-400">36EU - 4US</p>
                      </div>
  
                      <div class="mt-4 flex items-end justify-between sm:mt-0 sm:items-start sm:justify-end">
                        <p class="shrink-0 w-20 text-base font-semibold text-gray-900 sm:order-2 sm:ml-8 sm:text-right">RM150.00</p>
  
                        <div class="sm:order-1">
                          <div class="mx-auto flex h-8 items-stretch text-gray-600">
                            <button id="product2-decrementBtn" class="flex items-center justify-center rounded-l-md bg-gray-200 px-4 transition hover:bg-black hover:text-white" type="button">-</button>
                                <input id="product2-quantityInput" type="number" value="0" class="flex w-full items-center justify-center bg-gray-100 px-4 text-xs uppercase transition" style="text-align: center;" />
                                <button id="product2-incrementBtn" class="flex items-center justify-center rounded-r-md bg-gray-200 px-4 transition hover:bg-black hover:text-white" type="button">+</button>
                          </div>
                        </div>
                      </div>
                    </div>
  
                    <div class="absolute top-0 right-0 flex sm:bottom-0 sm:top-auto">
                      <button type="button" class="flex rounded p-2 text-center text-gray-500 transition-all duration-200 ease-in-out focus:shadow hover:text-gray-900">
                        <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" class=""></path>
                        </svg>
                      </button>
                    </div>
                  </div>
                </li>
                <li class="flex flex-col space-y-3 py-6 text-left sm:flex-row sm:space-x-5 sm:space-y-0">
                  <div class="shrink-0">
                    <img class="h-24 w-24 max-w-full rounded-lg object-cover" src="data:image/jpeg;base64," alt="" />
                  </div>
  
                  <div class="relative flex flex-1 flex-col justify-between">
                    <div class="sm:col-gap-5 sm:grid sm:grid-cols-2">
                      <div class="pr-8 sm:pr-5">
                        <p class="text-base font-semibold text-gray-900">Nike Air Max 2019</p>
                        <p class="mx-0 mt-1 mb-0 text-sm text-gray-400">36EU - 4US</p>
                      </div>
  
                      <div class="mt-4 flex items-end justify-between sm:mt-0 sm:items-start sm:justify-end">
                        <p class="shrink-0 w-20 text-base font-semibold text-gray-900 sm:order-2 sm:ml-8 sm:text-right">RM150.00</p>
  
                        <div class="sm:order-1">
                          <div class="mx-auto flex h-8 items-stretch text-gray-600">
                            <button id="product1-decrementBtn" class="flex items-center justify-center rounded-l-md bg-gray-200 px-4 transition hover:bg-black hover:text-white" type="button">-</button>
                            <input id="product1-quantityInput" type="number" value="0" class="flex w-full items-center justify-center bg-gray-100 px-4 text-xs uppercase transition" style="text-align: center;" />
                            <button id="product1-incrementBtn" class="flex items-center justify-center rounded-r-md bg-gray-200 px-4 transition hover:bg-black hover:text-white" type="button">+</button>
                          </div>
                      </div>
                      </div>
                    </div>
  
                    <div class="absolute top-0 right-0 flex sm:bottom-0 sm:top-auto">
                      <button type="button" class="flex rounded p-2 text-center text-gray-500 transition-all duration-200 ease-in-out focus:shadow hover:text-gray-900">
                        <svg class="block h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" class=""></path>
                        </svg>
                      </button>
                    </div>
                  </div>
                </li>
              </ul>
            </div>
  
            <div class="mt-6 border-t border-b py-2">
              <div class="flex items-center justify-between">
                <p class="text-sm text-gray-400">Tax %</p>
                <p  id="subtotalPrice" class="text-lg font-semibold text-gray-900">0%</p>
              </div>
            
            </div>
            <div class="mt-6 flex items-center justify-between">
              <p class="text-sm font-medium text-gray-900">Total</p>
              <p id="totalPrice" class="text-2xl font-semibold text-gray-900"><span class="text-xs font-normal text-gray-400"></span></p>
          </div>
          
            <div class="mt-6 text-center">
              <button type="submit" class="group inline-flex w-full items-center justify-center rounded-md bg-gray-900 px-6 py-4 text-lg font-semibold text-white transition-all duration-200 ease-in-out focus:shadow hover:bg-gray-800">
                Checkout
                <svg xmlns="http://www.w3.org/2000/svg" class="group-hover:ml-8 ml-4 h-6 w-6 transition-all" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M13 7l5 5m0 0l-5 5m5-5H6" />
                </svg>
              </button><br><br>
              <div class="mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex items-center justify-center">
                  <h1 class="text-1xl font-semibold text-gray-900">Developed By <a href="https://megatirfan.com" style="color: brown;">Megat Irfan</a></h1>
            </div>
            <input type="hidden" name="total_price" id="total_price" value="0">
          </form>
          </div>
        </div>
      </div>
    </div>
  </section>
  <script>
    
    function updateTotalPrice() {
        // Get the quantity input elements
        const quantityInput1 = document.getElementById('product1-quantityInput');
        const quantityInput2 = document.getElementById('product2-quantityInput');

        // Get the prices for each product
        const price1 = 150.00; // Replace with the actual price
        const price2 = 150.00; // Replace with the actual price

        // Calculate the new total price
         // Calculate the new total price
    const total = price1 * parseInt(quantityInput1.value) + price2 * parseInt(quantityInput2.value);

    // Update the total price element in the HTML
    const totalPriceElement = document.getElementById('totalPrice');
    totalPriceElement.textContent = `RM${total.toFixed(2)}`;

    // Update the hidden input value
    const total_price_input = document.getElementById('total_price');
    total_price_input.value = total.toFixed(2); // Update the hidden input field
    }

            
        

    // Attach event listeners to quantity input fields
    const quantityInput1 = document.getElementById('product1-quantityInput');
    const quantityInput2 = document.getElementById('product2-quantityInput');

    quantityInput1.addEventListener('input', updateTotalPrice);
    quantityInput2.addEventListener('input', updateTotalPrice);

    // Initialize the total price when the page loads
    updateTotalPrice();

    // Attach event listeners to increment and decrement buttons
    function attachEventListeners(productID) {
        const incrementBtn = document.getElementById(`${productID}-incrementBtn`);
        const decrementBtn = document.getElementById(`${productID}-decrementBtn`);

        incrementBtn.addEventListener('click', () => {
            const quantityInput = document.getElementById(`${productID}-quantityInput`);
            quantityInput.value = parseInt(quantityInput.value) + 1;
            updateTotalPrice(productID);
        });

        decrementBtn.addEventListener('click', () => {
            const quantityInput = document.getElementById(`${productID}-quantityInput`);
            if (parseInt(quantityInput.value) > 1) {
                quantityInput.value = parseInt(quantityInput.value) - 1;
                updateTotalPrice(productID);
            }
        });
    }

    // Attach event listeners for each product
    attachEventListeners('product1');
    attachEventListeners('product2');

    // Initialize total price
    updateTotalPrice('product1');
    updateTotalPrice('product2');
</script>

</body>
</html>